...
const NumberPad: React.FC<NumberPadProps> = ({ selectedNumber, theme, onNumberClick, onDeleteClick }) => {
  return (
    <div className={`w-full max-w-md mx-auto ${theme.cardBg} rounded-xl sm:rounded-2xl shadow-xl p-3 sm:p-4 border-2 ${theme.border}`}>
      <div className="grid grid-cols-5 gap-2 sm:gap-3">
          ...
          <button
            ...
            className={`aspect-square rounded-lg sm:rounded-xl font-bold text-xl sm:text-2xl transition-all duration-200 flex items-center justify-center
              ${selectedNumber === number 
                ? `${theme.numberButton} text-white shadow-lg scale-105` 
                : `${theme.button} ${theme.text} hover:scale-105`
              }`}
          >
            {number}
          </button>
        ...
        <button
          ...
          className={`aspect-square rounded-lg sm:rounded-xl font-bold text-lg sm:text-xl transition-all duration-200 flex items-center justify-center ${theme.button} ${theme.text} hover:scale-105`}
        >
